...
class PrimanotaForm(forms.ModelForm):
    ...
    class Meta:
        ...
        labels = {
            ...
            "codice_partita": "Cliente / Fornitore",
            "registro": "Registro",
    ...
    def clean_causale(self):
        ...
        if _tipo_is_generico(_form_tipo(self)):
            ...
            if not causale or not (causale.registro_iva or "").strip():
                raise forms.ValidationError(
                    "Per il tipo IVA scegliere una causale con registro IVA."
                )
        return code
...
class PrimanotaRigaForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "importo_iva": forms.NumberInput(attrs=_NUM),
    ...
    def linked_labels(self) -> dict[str, str]:
        ...
        return {
            "conto_partita": resolve_descrizione(
                "pdc", self._field_code("conto_partita", "conto_partita")
            ),
    ...
    def clean(self):
        ...
        if self.strict:
            if self.is_iva:
                ...
                if imponibile in (None, ""):
                    self.add_error("imponibile", "Campo obbligatorio.")
            else:
...
def _riga_vuota(cleaned: dict) -> bool:
    ...
    if any((cleaned.get(k) or "").strip() for k in keys):
        return False
    for key in ("dare", "avere", "imponibile", "importo_iva"):
        val = cleaned.get(key)
...
def riga_formset_for(data=None, queryset=None, *, is_iva: bool = False, **kwargs):
