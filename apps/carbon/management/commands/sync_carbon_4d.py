...
class Command(BaseCommand):
    help = (
        "Importa da 4D le tabelle CARBON: Reparti, Lavorazioni_Partite, "
        "TabStampi_Seriali_Partite."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help="Sincronizza solo una tabella (source o target name).",
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.WARNING("Avvio sync 4D -> PostgreSQL (CARBON)..."))

        result = sync_carbon(batch_size=batch_size, only=only, full=full)
